...
from familycli.database.db_manager import initialize_database
from familycli.ui.cli_interface import app, main_menu, welcome_screen
from familycli.database.backup_utils import backup_database, log_event, notify_user
from familycli.auth.user_manager import get_logged_in_user
...
# Configure logging to use centralized user directory
import os
from familycli.config.config_manager import UserDirectoryManager
user_dir_manager = UserDirectoryManager()
_LOG_FILE = os.path.join(user_dir_manager.get_logs_dir(), 'familycli.log')
...
def setup_application():
    ...
    try:
        ...
        # Run v1.1 migration if needed
        try:
            from familycli.database.migration_v1_1 import run_migration
            run_migration()
...
def main():
    ...
    # Check for --help flag before doing any complex imports
    if len(sys.argv) > 1 and sys.argv[1] in ['--help', '-h']:
        print("Family AI CLI - Interactive family chat simulation")
        print()
        print("Usage: familycli [OPTIONS] COMMAND [ARGS]...")
        print()
        print("Options:")
        ...
        print("  tree       Display family tree visualization")
        print("  add-pack   Install a persona pack")
        print("  scene      Start a storytelling scene")
        print("  feedback   View feedback statistics")
        print("  memory     View and manage persona memories")
        print("  reload     Reload configuration")
        print()
        print("Run without arguments to start the interactive menu.")
        return
    
    # If no --help flag, run the interactive application
    run_interactive()

def run_interactive():
    """Run the interactive application (called from the callback when no command is specified)."""
    try:
