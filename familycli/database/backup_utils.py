...
BACKUP_DIR = user_dir_manager.get_backups_dir()
LOG_FILE = os.path.join(user_dir_manager.get_logs_dir(), 'familycli.log')
...
def backup_database(db_path=None):
    if db_path is None:
        db_path = user_dir_manager.get_database_path()
...
def notify_user(message: str):
    ...
    try:
        ...
        notification_file = os.path.join(user_dir_manager.get_logs_dir(), 'notifications.log')
